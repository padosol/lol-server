...
> 상태: **구현 완료(인프라 준비 대기)** · 대상 컨텍스트: `module/domain/community`
>
> 코드는 브랜치 `feature/community-image-upload` 에 있다. `lol-db-schema` V36 은 머지됐고,
> 남은 것은 8절 PR 0(버킷·CloudFront·IAM)뿐이며 그것이 끝나면 바로 뜬다. 구현 과정에서
> 설계와 달라진 지점은 각 절에 **구현 노트**로 표시했다.
...
7. **Linear 이슈 번호** — 아직 발급받지 않아 브랜치·커밋에서 키를 뺐다. `<type>/MP-<번호>-*` 컨벤션을 지키려면 발급 후 되붙여야 한다.
