...
> 개정 2026-08-11 (문서 표 ↔ 실서비스 실호출 대조 보강) ·
...
**문서(C) 자신이 A·B 를 정본으로 가리킨다.** 언어 표에는 "Data Dragon languages file 에서도
받을 수 있다"며 `languages.json` 링크를, Regions 항목에는 "각 지역이 쓰는 버전은 realms 파일에서
확인할 수 있다"며 `realms/` 링크를 함께 싣는다. 즉 C 의 표는 A·B 의 **사본(스냅샷)** 이고,
지역별 버전이 다를 수 있음을 명시하며 그 확인처로 realms 를 안내한다. 따라서
**realms 파일이 "지역 ↔ 기본 언어" 의 정식 원천**이며, C 를 seed 로 쓰는 것은 문서의 안내를
거스르는 선택이다.
...
개수가 28개로 같아 **목록을 세는 것만으로는 차이를 발견할 수 없다.** 실제로 데이터를 받아
보면 갈린다 (2026-08-11, DDragon `16.15.1`, `cdn/{ver}/data/{locale}/champion.json`):

| 로케일 | 문서(C) 표 | 실호출 결과 | 판정 |
|---|---|---|---|
| `ms_MY` | 있음 | **HTTP 403**, 0 bytes | 문서에만 존재 — seed 하면 깨진다 |
| `ar_AE` | 없음 | HTTP 200, 251,766 bytes | 실서비스에만 존재 — 문서만 보면 누락된다 |
| `zh_MY` | 있음 | HTTP 200, 245,188 bytes | 정상 (말레이시아는 이쪽으로 서비스) |
| `ko_KR` | 있음 | HTTP 200, 265,383 bytes | 정상 (대조군) |

> `ms_MY`(말레이어)는 과거 목록에 있었으나 현재 DDragon 이 **403 으로 거부**하고, 반대로 중동
> 서버 오픈으로 추가된 `ar_AE`(아랍어)가 문서 표에는 반영되지 않았다.
...
**③ 플랫폼 라우팅 표: 문서 16개 + 문서에 없는 `ME1`**

문서(C)의 Platform 표는 16개다 — `BR1` `EUN1` `EUW1` `JP1` `KR` `LA1` `LA2` `NA1` `OC1`
`TR1` `RU` `PH2` `SG2` `TH2` `TW2` `VN2`. **`ME1` 은 표에 없고, 문서 전체에 `ME1`·`Middle East`·
`MENA`·`ar_AE`·`Arabic` 이 단 한 번도 등장하지 않는다.** 그러나 중동은 실재한다:

| 확인 대상 | 결과 | 해석 |
|---|---|---|
| `me1.api.riotgames.com` | **HTTP 401** | 호스트 존재 (인증만 없음) |
| `kr.api.riotgames.com` | HTTP 401 | 대조군 — 실재하는 호스트 |
| `zz9.api.riotgames.com` | **DNS 실패** | 대조군 — 없는 호스트는 401 조차 못 받는다 |
| `realms/me.json` | HTTP 200, `l=ar_AE`, `v=16.15.1` | 중동 realm 정상 동작 |

가짜 호스트가 DNS 단계에서 실패하므로 **401 은 "호스트가 실재한다" 는 신호**로 쓸 수 있다.

- 본 설계의 지역 seed 17개 = **문서 Platform 표 16개 전부 + 실측으로 확인한 `ME1`**.
  문서와 어긋나는 항목은 없고, 문서에 아직 없는 지역 하나를 더한 형태다.
- Regional routing 4종(`AMERICAS` `ASIA` `EUROPE` `SEA`)은 문서 표와 완전히 일치한다.
- 다만 **`ME1` 이 넷 중 어디로 라우팅되는지는 이 방법으로 확인할 수 없다** — 문서 표에 없고
  realms 응답에도 라우팅 정보가 없다. 인증된 키로 확인이 필요하다 ([§9-5](#9-열린-결정-사항)).
...
- 위 17개 중 `me` 를 뺀 16개는 **Riot 문서 Platform 표와 정확히 일치**한다 (§1.2 ③).
  `me` 만 문서에 없고 실호출로만 확인된 지역이다.

> **구현 전 재확인 필요**: `ME1` 의 regional routing 이 `EUROPE` 인지
> ([§9](#9-열린-결정-사항) 5번). 나머지 16개는 문서 Platform 표 · 현 `Platform` enum 값 양쪽과
> 대조해 확인했다. `ME1` 은 문서에 아예 없어 대조 대상이 없다.
...
| 5 | **`ME1` 의 regional routing** | `EUROPE` 로 seed. **인증된 API 키로 재확인 필요** — Riot 문서 Platform 표에 `ME1` 자체가 없고 realms 응답에도 라우팅 정보가 없어, 무인증으로는 확인할 수 있는 경로가 없다 (§1.2 ③) |
