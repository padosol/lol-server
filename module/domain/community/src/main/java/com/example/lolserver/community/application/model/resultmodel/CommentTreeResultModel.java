...
@Builder
@AllArgsConstructor
public class CommentTreeResultModel {
    ...
    @Builder.Default
    private List<CommentTreeResultModel> children = new ArrayList<>();

    public static CommentTreeResultModel of(Comment comment, MemberProfileReadModel author) {
        AuthorReadModel authorReadModel = author != null
                ? AuthorReadModel.of(author) : null;

        String content = comment.isDeleted()
                ? DELETED_COMMENT_CONTENT : comment.getContent();

        return CommentTreeResultModel.builder()
                .id(comment.getId())
