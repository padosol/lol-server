package com.example.lolserver.match;
...
/**
 * match 컨텍스트의 헥사고날 레이어 + 컨텍스트 경계 규칙.
 *
...
class ArchitectureTest {
    ...
    void application은_adapter에_의존하지_않는다() {
        ArchRule rule = noClasses()
                ...
                .resideInAPackage("..match.adapter..");
        rule.check(classes);
    }
    ...
    void in어댑터는_out어댑터에_직접_의존하지_않는다() {
        ArchRule rule = noClasses()
                .that().resideInAPackage("..match.adapter.in..")
                .should().dependOnClassesThat()
                .resideInAPackage("..match.adapter.out..")
    ...
    void 타_컨텍스트의_내부에_의존하지_않는다() {
        ...
        // match 는 리프 컨텍스트이므로 타 컨텍스트 패키지에 전혀 의존하지 않아야 한다.
        ArchRule rule = noClasses()
                ...
                .as("리프 컨텍스트 match 는 다른 컨텍스트에 의존하지 않는다");
        rule.check(classes);
    }
}
