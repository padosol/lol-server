...
class ChampionServiceTest {
    ...
    void getChampionRotate_캐시존재_캐시반환() {
        ...
        given(championPersistencePort.getChampionRotate(region)).willReturn(Optional.of(cachedRotate));

        // when
        ChampionRotate result = championService.getChampionRotate(region);

        // then
        assertThat(result).isEqualTo(cachedRotate);
        assertThat(result.getMaxNewPlayerLevel()).isEqualTo(10);
        then(championPersistencePort).should().getChampionRotate(region);
        then(championClientPort).should(never()).getChampionRotate(region);
    }
    ...
    void getChampionRotate_캐시없음_클라이언트조회후저장() {
        ...
        given(championPersistencePort.getChampionRotate(region)).willReturn(Optional.empty());
        given(championClientPort.getChampionRotate(region)).willReturn(newRotate);
        ...
        then(championPersistencePort).should().getChampionRotate(region);
        then(championClientPort).should().getChampionRotate(region);
        then(championPersistencePort).should().saveChampionRotate(region, newRotate);
    }
    ...
    void getChampionRotate_다른지역_정상조회() {
        ...
        given(championPersistencePort.getChampionRotate(region)).willReturn(Optional.of(naRotate));
