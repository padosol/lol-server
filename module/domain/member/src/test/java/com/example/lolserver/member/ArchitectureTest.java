package com.example.lolserver.member;
...
/**
 * member 컨텍스트의 헥사고날 레이어 + 컨텍스트 경계 규칙.
 *
...
class ArchitectureTest {
    ...
    private final JavaClasses classes = new ClassFileImporter()
            ...
            .importPackages("com.example.lolserver.member");

    @Test
    void domain은_application이나_adapter에_의존하지_않는다() {
        ArchRule rule = noClasses()
                .that().resideInAPackage("..member.domain..")
                .should().dependOnClassesThat()
                .resideInAnyPackage("..member.application..", "..member.adapter..");
        rule.check(classes);
    }
    ...
    void application은_adapter에_의존하지_않는다() {
        ArchRule rule = noClasses()
                .that().resideInAPackage("..member.application..")
                .should().dependOnClassesThat()
                .resideInAPackage("..member.adapter..");
        rule.check(classes);
    }
    ...
    void domain과_application은_인프라_기술타입에_의존하지_않는다() {
        ArchRule rule = noClasses()
                .that().resideInAnyPackage("..member.domain..", "..member.application..")
                .should().dependOnClassesThat()
                .resideInAnyPackage(
                        "jakarta.persistence..",
    ...
    void in어댑터는_out어댑터에_직접_의존하지_않는다() {
        ArchRule rule = noClasses()
                .that().resideInAPackage("..member.adapter.in..")
                .should().dependOnClassesThat()
                .resideInAPackage("..member.adapter.out..")
    ...
    void 타_컨텍스트의_내부에_의존하지_않는다() {
        ...
        // member 는 리프 컨텍스트이므로 타 컨텍스트 패키지에 전혀 의존하지 않아야 한다.
        ArchRule rule = noClasses()
                ...
                .as("리프 컨텍스트 member 는 다른 컨텍스트에 의존하지 않는다");
        rule.check(classes);
    }
}
